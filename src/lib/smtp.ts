...
export const sendPasswordResetEmail = async (email: string, otp: string, name?: string): Promise<boolean> => {
  try {
    const mailOptions = {
      from: `"Noor Al Quran" <noreply@thecharanjitsingh.com>`,
      to: email,
      subject: 'Reset Your Password - Noor Al Quran',
      html: `
        <!DOCTYPE html>
        <html lang="en">
        <head>
          <meta charset="UTF-8">
          <meta name="viewport" content="width=device-width, initial-scale=1.0">
          <title>Noor Al Quran - Password Reset</title>
        </head>
        <body style="margin: 0; padding: 0; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); min-height: 100vh;">
          <div style="max-width: 600px; margin: 0 auto; background: #ffffff; border-radius: 20px; overflow: hidden; box-shadow: 0 20px 40px rgba(0,0,0,0.1);">
            <!-- Header with Islamic Pattern -->
            <div style="background: linear-gradient(135deg, #e74c3c 0%, #c0392b 100%); padding: 40px 30px; text-align: center; position: relative;">
              <div style="position: absolute; top: 0; left: 0; right: 0; height: 100%; background-image: url('data:image/svg+xml,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100"><text fill="rgba(255,255,255,0.1)" font-size="20" y="50%">﷽</text></svg>'); background-repeat: repeat; opacity: 0.3;"></div>
              <h1 style="color: #ffffff; margin: 0; font-size: 28px; font-weight: 700; text-shadow: 2px 2px 4px rgba(0,0,0,0.3); position: relative; z-index: 2;">
                🔐 PASSWORD RESET
              </h1>
            ...
            <div style="padding: 40px 30px; text-align: center;">
              <div style="margin-bottom: 30px;">
                ...
                <p style="color: #7f8c8d; margin: 0; font-size: 16px; line-height: 1.6;">
                  We received a request to reset your password for Noor Al Quran. Use the code below to reset your password:
                </p>
              </div>

              <!-- OTP Card -->
              <div style="background: linear-gradient(135deg, #f8f9fa 0%, #e9ecef 100%); border: 2px solid #e74c3c; border-radius: 15px; padding: 30px; margin: 30px 0; position: relative; box-shadow: 0 8px 25px rgba(231, 76, 60, 0.2);">
                <div style="position: absolute; top: -15px; left: 50%; transform: translateX(-50%); background: #e74c3c; color: white; padding: 8px 20px; border-radius: 20px; font-size: 14px; font-weight: 600;">
                  🔑 RESET CODE
                </div>
                <div style="font-size: 42px; font-weight: 900; color: #2c3e50; letter-spacing: 8px; margin: 20px 0; text-shadow: 2px 2px 4px rgba(0,0,0,0.1); font-family: 'Courier New', monospace;">
                  ${otp}
              ...
              <!-- Instructions -->
              <div style="background: #f8f9fa; border-radius: 10px; padding: 20px; margin: 20px 0; border-left: 4px solid #e74c3c;">
                <h3 style="color: #2c3e50; margin: 0 0 10px 0; font-size: 18px;">📋 How to reset:</h3>
                <p style="color: #7f8c8d; margin: 0; font-size: 14px; line-height: 1.5;">
              ...
              <div style="border-top: 1px solid #ecf0f1; padding-top: 30px; margin-top: 30px;">
                <div style="text-align: center;">
                  ...
                  <div style="margin: 20px 0;">
                    <span style="display: inline-block; background: #e74c3c; color: white; padding: 8px 16px; border-radius: 20px; font-size: 12px; font-weight: 600;">
                      ﷽ Bismillah ﷽
                    </span>
                  </div>
                </div>
